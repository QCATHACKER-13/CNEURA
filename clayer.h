    ...
    public:
        ...
        void normalization (normtype ntype_norm){
            ...
            for(size_t i = 0; i < neuron.size(); i++){
                neuron[i].normalization(ntype_norm);
            }
        }

        void feedforward() {
            // Ensure input size matches the expected size
        
            for (size_t i = 0; i < neuron.size(); i++) {
                // Perform feedforward for each neuron
                neuron[i].feedforward();
        
                // Update layer outputs and weights
        ...
        void probability_calculation() {
            if (ntype == OUTPUT) {
                // Perform softmax for the output layer
                softmax();
                loss_function(); // Calculate loss
                loss_derivative(); // Calculate loss derivative
            } else if (ntype == HIDDEN) {
                // Propagate errors for hidden layers
                for (size_t i = 0; i < neuron.size(); ++i) {
                    error[i] = neuron[i].get_error(); // Get error from each neuron
                    }
            } else {
                throw logic_error("Invalid layer type for probability calculation.");
            }
        }
        ...
        void softmax() {
            ...
            for (size_t i = 0; i < activated_output.size(); ++i) {
                exp_values[i] = exp(activated_output[i] - maxVal);
            ...
            probability.resize(activated_output.size());
            for (size_t i = 0; i < activated_output.size(); ++i) {
        ...
        void loss_function() {
            ...
            for (size_t i = 0; i < neuron.size(); ++i) {
                ...
                switch (lossFunc) {
                    case MSE:
                        loss += 0.5 * diff * diff; // Mean squared error
                        break;
        
                    case BCE:
                        loss += -(probability_target[i] * log(probability[i]) +
                                  (1 - probability_target[i]) * log(1 - probability[i])) / probability.size();
                        break;
        
                    case CCE:
                        loss += -(probability_target[i] * log(probability[i])) / activated_output.size();
                        break;
        
                    case HUBER:
                        if (abs(diff) <= delta) {
                            loss += 0.5 * diff * diff; // Quadratic region
                        } else {
                            loss += delta * (abs(diff) - 0.5 * delta); // Linear region
                        }
                        break;
                }
            }
        ...
        void set_labeling(const vector<double>& labels, double resolution) {
